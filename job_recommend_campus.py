...
from config import (
    ...
    CAMPUS_SERVICE_PORT, SERVICE_HOST,
    WORK_TYPE_INTERNSHIP, WORK_NATURE_CAMPUS,
    RESUME_TYPE_CAMPUS, EMBEDDING_CONCURRENCY, MILVUS_CONCURRENCY,
    THRESHOLD, TOP_K_DEFAULT, CANDIDATE_LIMIT_DEFAULT, RRF_K_DEFAULT,
    RERANK_CUTOFF_MULTIPLIER
)
from common import (
    logger, LocationItem, JobInfoItem, _req_to_filters,
...
class ServiceState:
    ...
    async def initialize(self):
        ...
        logger.info("=== 实习校招服务初始化完成 ===")

    async def cleanup(self):
        """清理资源"""
        logger.info("=== 实习校招服务关闭，清理资源 ===")
        if self.http_session and not self.http_session.closed:
            await self.http_session.close()
        if self.db_pool:
            self.db_pool.close()
            await self.db_pool.wait_closed()
        logger.info("=== 实习校招服务资源清理完成 ===")
...
class SearchRequestSXXZ(BaseModel):
    ...
    user_id: int
    workNature: int
    jobInfo: Optional[List[JobInfoItem]] = None
...
# ----------------------------- 过滤条件构建（实习校招专用） -----------------------------
def _build_salary_ranges_filter_milvus(salary_ranges: Optional[List[int]]) -> Optional[str]:
    """构建薪资范围过滤条件"""
...
def _build_internship_months_filter_milvus(internship_months: Optional[List[int]]) -> Optional[str]:
    """构建实习月数过滤条件"""
...
def _build_is_remote_filter_milvus(is_remote: Optional[int]) -> Optional[str]:
    """构建是否远程过滤条件"""
    if is_remote == 1:
...
def build_milvus_expr(params: Dict[str, Any]) -> Optional[str]:
    ...
    return _and_expr((
        ...
        _build_salary_ranges_filter_milvus(params.get("salaryRanges")),
        build_education_filter_milvus(params.get("education")),
        _build_week_days_filter_milvus(params.get("weekWorkDays")),
        _build_internship_months_filter_milvus(params.get("internshipMonths")),
        _build_is_remote_filter_milvus(params.get("is_remote")),
...
# ----------------------------- Milvus 查询（实习校招专用） -----------------------------
async def milvus_hybrid_search_sxxz_async(
        collection,
        job_info: List[Dict],
    ...
    stage_size = 300
    offset_buffer = 300
    required_stages = (target_position + stage_size - 1) // stage_size
    candidate_limit = required_stages * stage_size + offset_buffer

    job_title_items = []
    for idx, item in enumerate(job_info):
        if isinstance(item, dict):
            title = item.get("jobTitle", "").strip()
            if title:
                job_title_items.append((title, idx))

    if not job_title_items:
        return []

    titles_list = _dedupe_keep_order([title for title, _ in job_title_items])
    embeddings = await embedding_client.embed_texts(titles_list)
    title_to_vec = {title: emb for title, emb in zip(titles_list, embeddings) if emb and len(emb) > 0}

    local_cache = _state.local_emb_cache
    search_requests = []

    for title, idx in job_title_items:
        vec = title_to_vec.get(title)
        if not vec:
            vec = local_cache.get(title) or local_cache.get("其他")
            if not vec:
                from config import EMBEDDING_DIM
                vec = [0.0] * EMBEDDING_DIM

        job_item = job_info[idx]
        locations = job_item.get("location")
        current_filters = filters.copy()
        if locations:
            current_filters["locations"] = locations

        filter_expr = build_milvus_expr(current_filters)
        ...
        try:
            ...
            logger.warning(f"BM25 request failed for {title}: {e}")
    ...
    async with _state._sem_milvus:
        ...
        all_ids = await loop.run_in_executor(None, _sync_hybrid_search)
...
async def search_sxxz_service_optimized(filters: Dict) -> Dict[str, Any]:
    ...
    resume_list = await get_resume_list_by_user_attachment(_state.db_pool, user_id=user_id,
                                                           resume_type=RESUME_TYPE_CAMPUS)
    logger.info(f"附件简历查询完成: 共{len(resume_list)}条记录")
    ...
    online_resume_list = await get_resume_list_by_user_online(_state.db_pool, user_id=user_id,
                                                              work_type=WORK_TYPE_INTERNSHIP)
    ...
    # 求职意向通道：多意向已在Milvus层RRF融合
    intent_best_scores = {}
    if job_info:
        intent_ids = await milvus_hybrid_search_sxxz_async(
            collection=collection,
            job_info=job_info,
            embedding_client=_state.embedding_client,
            filters=filters,
        ...
        if intent_ids:
            ...
            for r in reranked:
                intent_best_scores[r["job_id"]] = (r.get("final_score", 0), r)

    logger.info(f"重排完成 - 简历通道:{len(resume_best_scores)}个, 意向通道:{len(intent_best_scores)}个")

    # ========== 优化：改进的融合策略 ==========

    # 1. 跨通道分数归一化（解决打分尺度不一致问题）
    resume_norm = min_max_normalize(resume_best_scores)
    intent_norm = min_max_normalize(intent_best_scores)
    ...
    logger.info(
        f"融合完成 - 原始简历:{len(resume_best_scores)}, 原始意向:{len(intent_best_scores)}, "
    ...
    logger.info(f"实习校招推荐完成 - 总耗时: {total_time:.2f}s, 返回: {len(page_results)}个结果")
...
if __name__ == "__main__":
    ...
    uvicorn.run(app, host=SERVICE_HOST, port=CAMPUS_SERVICE_PORT, log_level="info")
